with expected as (
  ...
  select
    0 as idx,
    3 as comp,
    -0.005621682041773158 as factor
  union all
  select
    0 as idx,
    4 as comp,
    -0.012206344130984396 as factor
  union all
  select
    1 as idx,
    0 as comp,
  ...
  select
    1 as idx,
    3 as comp,
    0.00697611181135579 as factor
  union all
  select
    1 as idx,
    4 as comp,
    0.005492503032819722 as factor
  union all
  select
    2 as idx,
    0 as comp,
  ...
  select
    2 as idx,
    3 as comp,
    -0.015709405289296093 as factor
  union all
  select
    2 as idx,
    4 as comp,
    0.0015661183629507404 as factor
  union all
  select
    3 as idx,
    0 as comp,
  ...
  select
    3 as idx,
    3 as comp,
    0.008739882699523329 as factor
  union all
  select
    3 as idx,
    4 as comp,
    0.0037296993443871086 as factor
  union all
  select
    4 as idx,
    0 as comp,
  ...
  select
    4 as idx,
    3 as comp,
    0.0026710790470545896 as factor
  union all
  select
    4 as idx,
    4 as comp,
    0.008014744172554092 as factor
)
...
full outer join (
  ...
  select *
  from {{ ref("collinear_matrix_wide_input_factors_output") }}
  where idx <= 4
) as a
on a.comp = e.comp
and a.idx = e.idx
